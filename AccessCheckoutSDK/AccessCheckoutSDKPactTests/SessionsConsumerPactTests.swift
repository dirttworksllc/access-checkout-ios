...
class SessionsConsumerPactTests: XCTestCase {
    ...
    func testDiscoveryOfSessionsEndPoints() {
        ...
        let accessRootStub = ServiceStubs().get200(path: "", jsonResponse: accessRootJsonResponse(sessionsBaseUrl: pactServer.baseUrl))
        ...
        pactServer.run(timeout: 10) { testComplete in
            ServiceDiscoveryProvider.discover(baseUrl: accessRootStub.baseUrl) { result in
                switch result {
                case .success():
                    XCTAssertEqual(ServiceDiscoveryProvider.getSessionsCardEndpoint(), expectedCardSessionsEndpoint)
                    XCTAssertEqual(ServiceDiscoveryProvider.getSessionsCvcEndpoint(), expectedCvcSessionsEndpoint)
                case .failure:
    ...
    func testValidCvcSessionRequest_receivesCvcSession() {
        ...
        pactServer.run(timeout: 10) { testComplete in
            sessionsClient.createSession(baseUrl: self.pactServer.baseUrl, checkoutId: "identity", cvc: "1234") {
                result in
                switch result {
    ...
    private func stubServiceDiscovery() -> ServiceStubs? {
        ...
        _ = serviceStub
            .get200(path: "", jsonResponse: accessRootJsonResponse(sessionsBaseUrl: serviceStub.baseUrl))
            .get200(path: "/sessions", jsonResponse: sessionsRootJsonResponse(baseUrl: pactServer.baseUrl))
        serviceStub.start()
        ...
        while (attempts < maxAttempts) {
            if ServiceDiscoveryProvider.getSessionsCardEndpoint() != nil && ServiceDiscoveryProvider.getSessionsCvcEndpoint() != nil {
                return serviceStub;
            }
